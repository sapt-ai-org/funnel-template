...
/**
 * LogoLoop, from React Bits (reactbits.dev), TypeScript + Tailwind variant.
 *
 * Vendored close to upstream so a future update is a readable diff. Local
 * changes, each marked `local:`:
 *  - 'use client' (it runs a requestAnimationFrame loop and measures itself)
 *  - keyboard focus inside the loop pauses it exactly as hovering does, so a
 *    linked logo can be tabbed to without chasing a moving target
 *  - the upstream `(item as any)` casts and dependency-list hooks are kept;
 *    the lint rules they trip are switched off for this file only
 */
...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    // local: keyboard focus pauses the loop the same way a hovering pointer does.
